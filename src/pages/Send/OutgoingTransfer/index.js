...
import {getTransactions, internalTransfers, sendInternalTransfers, sendPayoutTransfers} from "../../../utils/userApi";
...
function OutgoingTransfer() {
    ...
    const handleSendInternalTransactions = async () => {
        ...
        const body = {
            comment: '',
            depositAddress: address, // получатель средств; cм. /account
            value: parseFloat(amount), // сумма перевода
            currencyId: id // см. /currencies
        }

        console.log(body, 'body')
        const {error, data} = await sendPayoutTransfers(body, webApp?.initData);
        ...
        if (error) {
            ...
            if (error?.kind && error?.message) {
                ...
                if (error?.message.includes("value must be a positive number")) {
                    setAmountError(true)
                }
            }
        ...
        if (data?.transactionId) {
            navigate(`/operation-completed?amount=${amount}&address=${address}&id=${id}&transactionId=${data?.transactionId}&chain-name=${chainName}&crypto-name=${cryptoName}`);
        }
                ...
                <div className="content">
                    ...
                    <form className="form" action="#">
                        ...
                        <div className="form__gorup">
                            ...
                            {/*<a className="form__sub" href="#">{translation?.selectFromAddressBook}</a>*/}
                        </div>
